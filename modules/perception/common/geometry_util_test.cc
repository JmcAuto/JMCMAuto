...
TEST_F(GeometryUtilTest, GetCloudBarycenter) {
  ...
  // case 1
  barycenter =
      GetCloudBarycenter<jmc_auto::perception::pcl_util::Point>(clouds_[0])
          .cast<float>();
  EXPECT_NEAR(14.188400, barycenter[0], EPSILON);
  // case 2
  barycenter =
      GetCloudBarycenter<jmc_auto::perception::pcl_util::Point>(clouds_[1])
          .cast<float>();
  EXPECT_NEAR(15.661365, barycenter[0], EPSILON);
  // case 3
  barycenter =
      GetCloudBarycenter<jmc_auto::perception::pcl_util::Point>(clouds_[2])
          .cast<float>();
  EXPECT_NEAR(29.376224, barycenter[0], EPSILON);
  // case 4
  barycenter =
      GetCloudBarycenter<jmc_auto::perception::pcl_util::Point>(clouds_[3])
          .cast<float>();
  EXPECT_NEAR(13.144600, barycenter[0], EPSILON);
  // case 5
  barycenter =
      GetCloudBarycenter<jmc_auto::perception::pcl_util::Point>(clouds_[4])
          .cast<float>();
