...
DEFINE_string(lidar_path, "/jmcauto/data/lidar/", "lidar path");
DEFINE_string(radar_path, "/jmcauto/data/radar/", "radar path");
 ...
 public:
  ...
  void GetSequentialSensorsDataFiles(std::vector<SensorFile>* sensors_files) {
    ...
    for (const auto& sensor_files_source : sensors_files_sources_) {
      ...
      if (!jmc_auto::common::util::DirectoryExists(source.folder_path)) {
...
int main(int argc, char** argv) {
  ...
  jmc_auto::perception::SequentialPerceptionTest test;
  if (!test.Init()) {
  ...
  std::vector<jmc_auto::perception::SensorFile> sensors_files;
  ...
  for (size_t i = FLAGS_start_frame; i < sensors_files.size(); ++i) {
    ...
    std::shared_ptr<jmc_auto::perception::SensorRawFrame> raw_frame(
        new jmc_auto::perception::SensorRawFrame);
