...
int ImageGpuPreprocessHandler::init(const std::string &intrinsics_path, int dev) {
    ...
    CUDA_CHECK_JMC_AUTO(cudaMalloc(&_d_rgb, _out_size)); // if input is rgb, its size is same as output's
    CUDA_CHECK_JMC_AUTO(cudaMalloc(&_d_dst, _out_size));
    ...
    CUDA_CHECK_JMC_AUTO(cudaMalloc(&_d_mapx, img_size * sizeof(float)));
    CUDA_CHECK_JMC_AUTO(cudaMalloc(&_d_mapy, img_size * sizeof(float)));

    CUDA_CHECK_JMC_AUTO(cudaMemcpy(_d_mapx, mapx.data(), img_size * sizeof(float),
                cudaMemcpyHostToDevice));
    CUDA_CHECK_JMC_AUTO(cudaMemcpy(_d_mapy, mapy.data(), img_size * sizeof(float),
                cudaMemcpyHostToDevice));
    _inited = true;
    return 0;
}
...
int ImageGpuPreprocessHandler::handle(uint8_t *src, uint8_t *dst) {
    ...
    CUDA_CHECK_JMC_AUTO(cudaMemcpy(_d_rgb, src, _in_size, cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK_JMC_AUTO(cudaMemcpy(dst, _d_dst, _out_size, cudaMemcpyDeviceToHost));
    return 0;
}
...
int ImageGpuPreprocessHandler::release(void) {
    if (_d_mapy) {
        CUDA_CHECK_JMC_AUTO(cudaFree(_d_mapy));
        _d_mapy = NULL;
    }
    if (_d_mapx) {
        CUDA_CHECK_JMC_AUTO(cudaFree(_d_mapx));
        _d_mapx = NULL;
    }
    if (_d_dst) {
        CUDA_CHECK_JMC_AUTO(cudaFree(_d_dst));
        _d_dst = NULL;
    }
    if (_d_rgb) {
        CUDA_CHECK_JMC_AUTO(cudaFree(_d_rgb));
        _d_rgb = NULL;
    }
    _inited = false;
    return 0;
