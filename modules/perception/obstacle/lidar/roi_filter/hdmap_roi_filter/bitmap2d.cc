...
void Bitmap2D::Set(const size_t x_id, const size_t min_y_id,
  ...
  AINFO<<"left_block_id:"<<left_block_id<<"left_bit_id:"<<left_bit_id<<"right_block_id:"<<right_block_id<<"right_bit_id:"<<right_bit_id;
  auto& blocks = bitmap_[x_id];
  AINFO<<"x_id:"<<x_id<<"min_y_id:"<<min_y_id<<"max_y_id:"<<max_y_id;
...
inline void Bitmap2D::SetUint64RangeBits(const size_t head, const size_t tail,
                                         uint64_t* block) {
  AINFO<<"all_ones:"<<all_ones<<"block:"<<block<<"head:"<<head<<"tail"<<tail;
...
void Bitmap2D::BuildMap() {
  ...
  AINFO<<"Bitmap2D rows:"<<rows<<" Bitmap2D cols:"<<cols;
  bitmap_ =
