...
bool YoloCameraDetector::Init(const CameraDetectorInitOptions &options) {
  ...
  const string yolo_config = jmc_auto::common::util::GetAbsolutePath(
      yolo_root, FLAGS_yolo_config_filename);

  const string lane_config = jmc_auto::common::util::GetAbsolutePath(
      yolo_root, "lane.pt");
  CHECK(jmc_auto::common::util::GetProtoFromASCIIFile(yolo_config, &yolo_param_));
  CHECK(jmc_auto::common::util::GetProtoFromASCIIFile(lane_config, &lane_param_));
...
void YoloCameraDetector::init_anchor(const string &yolo_root) {
  ...
  string model_root = jmc_auto::common::util::GetAbsolutePath(
      yolo_root, model_param.model_name());

  // loading anchors
  string anchors_file = jmc_auto::common::util::GetAbsolutePath(
  ...
  // loading types
  string types_file = jmc_auto::common::util::GetAbsolutePath(
...
bool YoloCameraDetector::init_cnn(const string &yolo_root) {
  ...
  string model_root = jmc_auto::common::util::GetAbsolutePath(
      yolo_root, model_param.model_name());
  string proto_file = jmc_auto::common::util::GetAbsolutePath(
      model_root, model_param.proto_file());
  string weight_file = jmc_auto::common::util::GetAbsolutePath(
      model_root, model_param.weight_file());
  string feature_file = jmc_auto::common::util::GetAbsolutePath(
  ...
  CHECK(jmc_auto::common::util::GetProtoFromASCIIFile(feature_file, &feat_param));
  ...
  if (feat_param.has_remap_model()) {
    ...
    track_model =
        jmc_auto::common::util::GetAbsolutePath(model_root, track_model);
