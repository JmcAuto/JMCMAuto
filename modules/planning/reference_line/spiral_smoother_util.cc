...
class SpiralSmootherUtil {
 public:
  ...
  static bool Smooth(std::vector<Eigen::Vector2d> raw_points,
    ...
    ACHECK(jmc_auto::common::util::GetProtoFromFile(
...
int main(int argc, char* argv[]) {
  ...
  std::vector<Eigen::Vector2d> raw_points =
      jmc_auto::planning::SpiralSmootherUtil::ReadCoordinatesFromFile(
          FLAGS_input_file);

  std::vector<jmc_auto::common::PathPoint> smooth_points;
  auto res =
      jmc_auto::planning::SpiralSmootherUtil::Smooth(raw_points, &smooth_points);
  if (!res) {
  ...
  jmc_auto::planning::SpiralSmootherUtil::Export(FLAGS_output_file,
                                               smooth_points);
  return 0;
}
