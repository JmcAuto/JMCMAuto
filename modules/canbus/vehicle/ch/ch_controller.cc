...
// using jmc_auto::common::config::VehicleConfigHelper;
namespace {
...
ErrorCode ChController::EnableAutoMode() {
  ...
  if (!CheckResponse(flag, true)) {
    ...
    AINFO<<"Failed to switch to COMPLETE_AUTO_DRIVE mode.";
    Emergency();
...
ErrorCode ChController::DisableAutoMode() {
  // brake_command_111_->set_brake_pedal_en_ctrl(
  //     Brake_command_111::BRAKE_PEDAL_EN_CTRL_DISABLE);
  // throttle_command_110_->set_throttle_pedal_en_ctrl(
  //     Throttle_command_110::THROTTLE_PEDAL_EN_CTRL_DISABLE);
  // steer_command_112_->set_steer_angle_en_ctrl(
  //     Steer_command_112::STEER_ANGLE_EN_CTRL_DISABLE);
  ResetProtocol();
  can_sender_->Update();
...
ErrorCode ChController::EnableAPAMode() {}
ErrorCode ChController::DisableAPAMode() {}
void ChController::SteerTorque(double torque) {};
void ChController::PamStopDistance(int distance) {};
void ChController::SpeedTarget(float speed){};
...
ErrorCode ChController::EnableSpeedOnlyMode() {
  if (driving_mode() == Chassis::AUTO_SPEED_ONLY) {
...
if (driving_mode() == Chassis::REMOTE_MODE) {
  ...
  const int32_t flag =
      CHECK_RESPONSE_STEER_UNIT_FLAG | CHECK_RESPONSE_SPEED_UNIT_FLAG;
  if (!CheckResponse(flag, true)) {
    AERROR << "Failed to switch to REMOTE_MODE mode.";
    Emergency();
    set_chassis_error_code(Chassis::CHASSIS_ERROR);
    return ErrorCode::CANBUS_ERROR;
  } else {
    set_driving_mode(Chassis::REMOTE_MODE);
...
void ChController::Gear(Chassis::GearPosition gear_position) {
  if (!(driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
        ...
        driving_mode() == Chassis::REMOTE_MODE)) {
...
void ChController::Steer(double angle) {
  ...
  // reverse sign
  // ADD YOUR OWN CAR CHASSIS OPERATION
  steer_command_112_->set_steer_angle_cmd(static_cast<double>(real_angle));
}
...
void ChController::SecurityDogThreadFunc()
{
	...
	bool IS_RESPONSE_STEER = false;
	bool IS_RESPONSE_SPEED = false;
	bool IS_CHASSIS_ERROR = false;
	ChassisDetail chassis_detail;
	...
	while (can_sender_->IsRunning())
	{
		...
		else
		{
			...
			IS_RESPONSE_SPEED = true;
		}
		if (CheckChassisError())
		{
			set_chassis_error_code(Chassis::CHASSIS_ERROR);
			emergency_mode = true;
			IS_CHASSIS_ERROR = true;
		}
		else
		{
			IS_CHASSIS_ERROR = false;
		}

    // if (chassis_detail.ch().ecu_status_1_515().speed() == 0&&chassis_detail.ch().brake_command_111().brake_pedal_en_ctrl() == Brake_command_111::BRAKE_PEDAL_EN_CTRL_DISABLE && chassis_detail.ch().brake_command_111().brake_pedal_cmd() != 0)
		// 	{
		// 		DisableAutoMode();
		// 	}

		if (mode== Chassis::EMERGENCY_MODE&&IS_RESPONSE_SPEED && IS_RESPONSE_STEER && !IS_CHASSIS_ERROR)
		{
			// if (chassis_detail.ch().brake_command_111().brake_pedal_en_ctrl() == Brake_command_111::BRAKE_PEDAL_EN_CTRL_DISABLE && chassis_detail.ch().brake_command_111().brake_pedal_cmd() != 0)
			// {
			// 	DisableAutoMode();
			// }
			// else
...
//    }
//
//    if (emergency_mode && !(mode == Chassis::EMERGENCY_MODE||mode == Chassis::AUTO_SPEED_ONLY)) {
//      set_driving_mode(Chassis::EMERGENCY_MODE);
//      message_manager_->ResetSendMessages();
//      AERROR << "EMERGENCY_MODE:";
//    }
