 ...
 * essential components in JmcAuto, such as localization and control are
 * examples of JmcAuto apps. The JMC_AUTO_MAIN macro helps developer to setup
...
class JmcAutoApp {
  ...
  protected:
    /**
     * @brief The module initialization function. This is the first function
     * being called when the App starts. Usually this function loads the
     * configurations, subscribe the data from sensors or other modules.
     * @return Status initialization status
     */
    virtual jmc_auto::common::Status Init() = 0;

    /**
     * @brief The module start function. JmcAuto app usually triggered to
     * execute in two ways: 1. Triggered by upstream messages, or 2. Triggered
     * by timer. If an app is triggered by upstream messages, the Start()
     * function usually register a call back function that will be called when
     * an upstream message is received. If an app is triggered by timer, the
     * Start() function usually register a timer callback function.
