...
TEST(MessageTraitsTest, message_type) {
  ...
  EXPECT_EQ(msg_type, "jmc_auto.cyber.proto.UnitTest");
...
TEST(MessageTraitsTest, descriptor) {
  const std::string pb_desc =
      ...
      "\x1(\x4\x12\xF\n\acontent\x18\x3 \x1(\t";
  std::string desc;
  GetDescriptorString<proto::UnitTest>("jmc_auto.cyber.proto.UnitTest", &desc);
  EXPECT_EQ(pb_desc, desc);

  desc = "";
  GetDescriptorString<RawMessage>("jmc_auto.cyber.proto.UnitTest", &desc);
  EXPECT_EQ(pb_desc, desc);

  desc = "";
  GetDescriptorString<Data>("jmc_auto", &desc);
  EXPECT_EQ("", desc);

  desc = "";
  GetDescriptorString<Message>("jmc_auto", &desc);
