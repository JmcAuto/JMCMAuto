...
#define REGISTER_ADAPTER(name)
    ...
    private:
      ...
      std::unique_ptr<jmc_auto::skeleton::name##ServiceInterfaceSkeleton> name##skeleton = nullptr;
      std::unique_ptr<jmc_auto::proxy::name##ServiceInterfaceProxy> name##proxy = nullptr;
      AdapterConfig name##config_;

      void InternalEnable##name(const uint32_t &instance_id,
                                const AdapterConfig &config)
      {
        name##_.reset(
            new name##Adapter(#name, instance_id, config.message_history_limit()));
        if (config.mode() != AdapterConfig::PUBLISH_ONLY)
        {
          //测试
          jmc_auto::proxy::name##ServiceInterfaceProxy::StartFindService(
              [this](ara::com::ServiceHandleContainer<jmc_auto::proxy::name##ServiceInterfaceProxy::HandleType> handles, ara::com::FindServiceHandle handler) {
                instance()->ServiceAvailabilityCallback##name(std::move(handles), handler);
              },
              instance_id);
        }
        if (config.mode() != AdapterConfig::RECEIVE_ONLY)
        {
          name##skeleton = std::make_unique<jmc_auto::skeleton::name##ServiceInterfaceSkeleton>(
      ...
      void ServiceAvailabilityCallback##name(ara::com::ServiceHandleContainer<jmc_auto::proxy::name##ServiceInterfaceProxy::HandleType> handles,
                                             ara::com::FindServiceHandle handler)
      {
        if (handles.size() > 0)
        {
          for (unsigned int i = 0; i < handles.size(); i++)
          {
            ...
            {
              // 获取服务处理的句柄
              name##proxy = std::make_unique<jmc_auto::proxy::name##ServiceInterfaceProxy>(handles[i]);
              name##proxy->name##Event.Subscribe(ara::com::EventCacheUpdatePolicy::kNewestN, config.message_history_limit());
              // 注册数据接收的回调函数FusionPublishEventReceived
              name##proxy->name##Event.SetReceiveHandler(
                  [this]() { instance()->name##PublishEventCallback(); });
            }
          }
        }
      }
      ...
      void name##PublishEventCallback()
      {
        ...
        const auto &name##MsgSamples = name##proxy->name##Event.GetCachedSamples();
