...
class CanClient {
  public:
     ...
     */
    virtual jmc_auto::common::ErrorCode Start() = 0;
     ...
     *         jmc_auto::common::ErrorCode.
     */
    virtual jmc_auto::common::ErrorCode
    Send(const std::vector<CanFrame> &frames, int32_t *const frame_num) = 0;

    /**
     * @brief Send a single message.
     * @param frames A single-element vector containing only one message.
     * @return The status of the sending single message action which is defined
     * by jmc_auto::common::ErrorCode.
     */
    virtual jmc_auto::common::ErrorCode
    SendSingleFrame(const std::vector<CanFrame> &frames) {
        CHECK_EQ(frames.size(), 1)
            << "frames size not equal to 1, actual frame size :"
            << frames.size();
        int32_t n = 1;
     ...
     */
    virtual jmc_auto::common::ErrorCode
    Receive(std::vector<CanFrame> *const frames, int32_t *const frame_num) = 0;
