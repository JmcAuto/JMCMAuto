...
inline std::string EndWayPointFile() {
  if (FLAGS_use_navigation_mode) {
    return jmc_auto::common::util::StrCat(
        FLAGS_navigation_mode_end_way_point_file);
  } else {
    return jmc_auto::common::util::StrCat(FLAGS_map_dir, "/",
 ...
 */
inline jmc_auto::hdmap::Id MakeMapId(const std::string& id) {
  jmc_auto::hdmap::Id map_id;
  map_id.set_id(id);
  return map_id;
}
