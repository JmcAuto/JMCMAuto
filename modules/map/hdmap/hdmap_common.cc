...
jmc_auto::common::math::Polygon2d ConvertToPolygon2d(const Polygon &polygon) {
  ...
  while (points.size() >= 2 && points[0].DistanceTo(points.back()) <=
                                   jmc_auto::common::math::kMathEpsilon) {
    points.pop_back();
  }
  return jmc_auto::common::math::Polygon2d(points);
}

void SegmentsFromCurve(
    const Curve &curve,
    std::vector<jmc_auto::common::math::LineSegment2d> *segments) {
...
bool LaneInfo::IsOnLane(const jmc_auto::common::math::Box2d &box) const {
...
PointENU LaneInfo::GetSmoothPoint(double s) const {
  ...
  if (delta_s < jmc_auto::common::math::kMathEpsilon) {
...
void LaneInfo::CreateKDTree() {
  jmc_auto::common::math::AABoxKDTreeParams params;
  ...
  for (size_t id = 0; id < segments_.size(); ++id) {
    ...
    segment_box_list_.emplace_back(
        jmc_auto::common::math::AABox2d(segment.start(), segment.end()), this,
        &segment, id);
  }
