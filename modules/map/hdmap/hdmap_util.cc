...
std::string FindFirstExist(const std::string& dir, const std::string& files) {
  const auto candidates =
      jmc_auto::common::util::StringTokenizer::Split(files, "|");
  for (const auto& filename : candidates) {
    const std::string file_path =
        jmc_auto::common::util::StrCat(FLAGS_map_dir, "/", filename);
    if (jmc_auto::common::util::PathExists(file_path)) {
      return file_path;
    }
  }
  ...
  return jmc_auto::common::util::StrCat(FLAGS_map_dir, "/", candidates[0]);
}
