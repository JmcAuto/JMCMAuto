...
class NavigationLane {
 public:
  NavigationLane() = default;//构造函数
  explicit NavigationLane(const NavigationLaneConfig& config);
  ~NavigationLane() = default;//析够函数

  void SetConfig(const NavigationLaneConfig& config);//设置配置

  void SetDefaultWidth(const double left_width, const double right_width) {
    default_left_width_ = left_width;
    default_right_width_ = right_width;//设置默认车道宽度
  }

  bool GeneratePath();//生成路径

  void UpdatePerception(//更新感知
      const perception::PerceptionObstacles& perception_obstacles) {
    perception_obstacles_ = perception_obstacles;
  }

  void UpdateNavigationInfo(const NavigationInfo& navigation_info) {//更新导航信息
...
//转换导航线路为路径
  ...
  // received from topic: /jmc_auto/perception_obstacles
  perception::PerceptionObstacles perception_obstacles_;//感知障碍物对象

  // received from topic: /jmc_auto/navigation
  NavigationInfo navigation_info_;//导航信息对象
  ...
  double default_left_width_ = 1.75;//默认左边宽度
  double default_right_width_ = 1.75;//默认右边宽度
