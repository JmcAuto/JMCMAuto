...
using jmc_auto::common::Path;
using jmc_auto::common::VehicleStateProvider;
using jmc_auto::common::math::Vec2d;
using jmc_auto::common::util::DistanceXY;
using jmc_auto::hdmap::Lane;
...
bool NavigationLane::GeneratePath() {//生成路径
  navigation_path_list_.clear();//先清除
...
bool NavigationLane::CreateMap(const MapGenerationParam &map_config,
  ...
  left_edge->set_type(jmc_auto::hdmap::BoundaryEdge::LEFT_BOUNDARY);
  left_edge->mutable_curve()->CopyFrom(hdmap->lane(0).left_boundary().curve());

  auto *right_edge = outer_polygon->add_edge();
  right_edge->set_type(jmc_auto::hdmap::BoundaryEdge::RIGHT_BOUNDARY);
