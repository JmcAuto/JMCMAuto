...
static void OutputMap(const Map& map_pb) {
  ...
  CHECK(jmc_auto::common::util::SetProtoToASCIIFile(map_pb, txt_file));
  CHECK(jmc_auto::common::util::SetProtoToBinaryFile(map_pb, bin_file));
}
...
int main(int32_t argc, char** argv) {
  ...
  Map map_pb;
  const auto map_file = jmc_auto::hdmap::BaseMapFile();
  CHECK(jmc_auto::common::util::GetProtoFromFile(map_file, &map_pb))
