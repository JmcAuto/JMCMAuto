...
using jmc_auto::hdmap::LaneBoundary;
using jmc_auto::hdmap::LaneBoundaryType;
...
std::ostream &operator<<(
    std::ostream &os,
    const ::google::protobuf::RepeatedPtrField<jmc_auto::hdmap::Id> &ids) {
...
class MapUtil {
 public:
  ...
  int LaneProjection(const jmc_auto::common::math::Vec2d &vec2d,
...
int main(int argc, char *argv[]) {
  ...
  const std::string map_file = jmc_auto::hdmap::BaseMapFile();
  bool valid_arg = false;

  jmc_auto::hdmap::MapUtil map_util;
  ...
  if (!FLAGS_dump_txt_map.empty()) {
    jmc_auto::hdmap::Map map;
    CHECK(jmc_auto::common::util::GetProtoFromFile(map_file, &map));
    CHECK(jmc_auto::common::util::SetProtoToASCIIFile(map, FLAGS_dump_txt_map));
    valid_arg = true;
  }
  if (!FLAGS_dump_bin_map.empty()) {
    jmc_auto::hdmap::Map map;
    CHECK(jmc_auto::common::util::GetProtoFromFile(map_file, &map));
    CHECK(jmc_auto::common::util::SetProtoToBinaryFile(map, FLAGS_dump_bin_map));
    valid_arg = true;
  }
  if (!valid_arg) {
