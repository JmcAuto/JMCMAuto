...
int main(int32_t argc, char** argv) {
  ...
  jmc_auto::common::math::EulerAnglesZXY<double> euler(
      FLAGS_qw, FLAGS_qx, FLAGS_qy, FLAGS_qz);
  AINFO << "roll: " << euler.roll() << " pitch:" << euler.pitch()
        << " yaw:" << euler.yaw();
  AINFO << "heading: " << jmc_auto::common::math::QuaternionToHeading(
                              FLAGS_qw, FLAGS_qx, FLAGS_qy, FLAGS_qz)
        << " heading degree: "
        << 180.0 / M_PI * jmc_auto::common::math::QuaternionToHeading(
                              FLAGS_qw, FLAGS_qx, FLAGS_qy, FLAGS_qz);
}
