...
DEFINE_string(container_meta_ini, "/jmcauto/meta.ini",
...
Map<std::string, std::string> LoadFiles(
  ...
  for (const auto &file : files) {
    if (jmc_auto::common::util::GetContent(file, &content)) {
...
InfoCollector::InfoCollector() {
  ...
  for (auto& conf_file : *config_.mutable_hardware_configs()) {
    conf_file = jmc_auto::common::util::TranslatePath(conf_file);
  }
...
const VehicleInfo &InfoCollector::GetVehicleInfo() {
  ...
  const std::string vehicle_name = KVDB::Get("jmc_auto:dreamview:vehicle");
  if (!vehicle_name.empty()) {
    vehicle->set_name(vehicle_name);
  }

  const std::string vehicle_vin = KVDB::Get("jmc_auto:canbus:vin");
...
const EnvironmentInfo &InfoCollector::GetEnvironmentInfo() {
  ...
  const std::string map_name = KVDB::Get("jmc_auto:dreamview:map");
  if (!map_name.empty()) {
...
const SoftwareInfo &InfoCollector::GetSoftwareInfo() {
  ...
  const std::string commit_id = KVDB::Get("jmc_auto:data:commit_id");
  if (!commit_id.empty()) {
    software->set_commit_id(commit_id);
  }

  const std::string mode_name = KVDB::Get("jmc_auto:dreamview:mode");
  if (!mode_name.empty()) {
...
std::string InfoCollector::GetDockerImage() {
  ...
  if (jmc_auto::common::util::PathExists(FLAGS_container_meta_ini)) {
