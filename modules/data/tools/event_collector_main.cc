...
DEFINE_string(adapter_config_filename,
              "/jmcauto/modules/data/conf/event_collector_adapter.conf",
...
class EventCollector {
 public:
  ...
  void Stop() {
    ...
    for (const std::string& bag_filename :
         jmc_auto::common::util::ListSubPaths("./", DT_REG)) {
      if (!jmc_auto::common::util::EndWith(bag_filename, ".bag")) {
        continue;
      }
 ...
 private:
  ...
  void OnDriveEvent(const jmc_auto::common::DriveEvent& event) {
  ...
  void OnMonitorMessage(
      const jmc_auto::common::monitor::MonitorMessage& monitor_msg) {
  ...
  void SaveEvent(const double timestamp_sec, const std::string& type,
    ...
    const auto msg = jmc_auto::common::util::StrCat(
...
int main(int32_t argc, char **argv) {
  ...
  jmc_auto::data::EventCollector event_collector;
